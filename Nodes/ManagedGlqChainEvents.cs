...
{
    public class ManagedGlqChainEvents
    {
        ...
        public ManagedGlqChainEvents(StreamingWebSocketClient socket, Web3 web3Client)
        {
            SocketClient = socket;
        ...
        private void ResetEvent(string type, dynamic eventObject)
        {
            try
            {
                ...
                Console.WriteLine(string.Format("[Managed GLQ Chain]: '{0}' event cleared, no new nodes attached to this event", type));
            }
            catch (Exception error)
            {
                Console.WriteLine(error);
